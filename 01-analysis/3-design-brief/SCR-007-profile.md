...
- **Справочные пункты** — правила центра, контакты поддержки, версия приложения.
...
| **Контекст** | Чаще всего — у трассы/на солнце, руки в перчатках или холодные (см. [foundations §1](00-foundations.md#1-продукт-и-аудитория)). Заход в «Профиль» обычно эпизодический: свериться с данными или выйти на чужом/общем устройстве. |
...
**Заезды** ([SCR-002](SCR-002-slot-list.md)) · **Мои записи** ([SCR-005](SCR-005-my-bookings.md)) ·
...
сущность клиента из [модели данных](../4-design/data-model).
...
| Справочные пункты | «Правила центра», «Поддержка», «Версия приложения» | конфиг / статика | read; правила/поддержка — переходы, версия — текст |
...
3. **Справочные пункты** — «Правила центра» ›, «Поддержка» ›, «Версия приложения».
...
- **Обязательное подтверждение** перед выходом — защита от случайного нажатия «рукой в перчатке»
  у трассы (P1). Подтверждение оформляется как диалог/шторка по правилам
...
- **Правила центра** и **Поддержка** — строки-переходы (›) на статические/внешние ресурсы (адрес/ссылка — параметр конфигурации).
...
| Пояснение | «Чтобы снова записаться на заезд, нужно будет войти по номеру телефона.» |
...
3. **Справочные пункты — да.** Правила центра, поддержка, версия приложения (§6.4); адреса/ссылки — параметр конфигурации.
