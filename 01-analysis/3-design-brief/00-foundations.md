...
[НФТ](../2-requirements/non-functional-requirements) ·
[ФТ](../2-requirements/functional-requirements) ·
...
уличного картинг-центра. Заменяет запись через Telegram и доску маркером.

**Единственная роль — «Клиент».** Маршал и владелец в приложение не входят. Справочные
данные (слоты, конфигурации трасс, маршалы) — read-only из API. Оплата — **офлайн** (наличные /
перевод); приложение показывает цену и фиксирует запись, онлайн-оплаты нет.

**Контекст использования:** клиент стоит у трассы, нередко на солнце, руки могут быть
в перчатках, спешит перед заездом. Это диктует крупные элементы, высокий
...
|---|---------|----------|---------------------------|
| P1 | **Mobile-first для трассы** | NFR-1 | Крупные тач-зоны, высокий контраст, читаемость на ярком солнце, минимум мелкого текста. |
...
| P6 | **Честность и спокойствие** | UC-1/UC-2 | Ошибки и правила (места, экипировка, 2 часа) объясняются понятно и без давления; штрафов нет. |
...
- Между кликабельными элементами — отступ, исключающий промахи «в перчатках».
...
- **Заезды** ([SCR-002](SCR-002-slot-list.md)) — стартовая вкладка (список доступных заездов).
...
### 4.5 Карта трассы (статичный превью Яндекс.Карт)

Сквозной компонент для экранов, где показывается трасса ([SCR-003](SCR-003-slot-card.md),
[SCR-006](SCR-006-booking-details.md)). Описывается здесь один раз; экраны на него **ссылаются**.

```
┌─────────────────────────────────┐
│  ░░░░░ карта (статичный превью) ░│
│  ░░░╱‾‾‾╲░░ выделенная линия ░░│  ← полилиния трассы (slot.geometry)
│  ░░      ╲___░░░  ░░░░░░░░░░░│  ← пин места сбора (meeting_point)
│  [ Открыть карту › ]            │  ← тап → BS-004 (интерактивная карта)
├─────────────────────────────────┤
│  📍 Место сбора                 │  ← текстовый блок под картой
│  Картинг-центр «Апекс», въезд 1 │     (адрес/ориентир, meeting_point)
└─────────────────────────────────┘
```

- **Тип:** **статичное изображение** карты (Яндекс Static API) с **выделенной линией трассы**
  (полилиния из `slot.geometry`) и **пином места сбора** (`slot.meeting_point` + координаты).
  По умолчанию — статика, а не тяжёлый интерактив (трасса, трафик/батарея, P1/P4).
- **Тап по карте** → шторка [BS-004 «Карта трассы»](BS-004-track-map.md) с **открытой
  интерактивной картой** и отрисованной трассой (зум/пан). Внутри BS-004 — действия «Открыть в
  Яндекс.Картах» и «Проложить маршрут» (handoff во внешнее приложение).
- **Текстовый блок места сбора** под картой обязателен — он же служит текстовым эквивалентом
  (адрес/ориентир), считывается без карты (см. §7).
- **Состояния:**
  - *Loading* — скелетон в форме карты (не пустой блок).
  - *Error / offline / нет ключа карты* — **fallback на текст**: блок места сбора + ссылка
...
> Числовые лимиты (потолок конфигурации трассы, размер прокатного фонда) **не зашиваются в тексты** —
> подставляются из данных слота/конфигурации.
>
> **Раздельная модель доступности (R-003).** Места и прокат считаются **независимо**:
> - **Места** (за раз можно выбрать): `max_seats = min(free_seats, track_config.capacity_cap, 4)`.
> - **Прокатная экипировка**: ограничение отдельное — `rental_count ≤ free_rental_equipment`; своя
>   экипировка не расходует прокатный фонд.
>
> Прокатная экипировка **не** ограничивает число доступных мест — формулу доступности «через
> прокатную экипировку» использовать **нельзя**.
...
- **Карта трассы (§4.5) — не единственный носитель информации:** место сбора и трасса
  обязательно продублированы **текстом** (адрес/ориентир + название и тип конфигурации трассы). У карты —
...
- Заблаговременное напоминание о предстоящем заезде; уведомление об отмене заезда
  центром/маршалом.
...
| Термин | Значение |
|--------|----------|
| **Заезд / Слот** | Конкретный групповой заезд: дата, время старта, конфигурация трассы, маршал, цена, всего/свободно мест. |
| **Конфигурация трассы** | Вариант заезда (новичковая короткая / опытная длинная); свой потолок мест и длительность. |
| **Экипировка** | Шлем и подшлемник: прокатная (из фонда центра) или собственная (клиент со своей). |
...
| BS-004 | [Карта трассы](BS-004-track-map.md) |
