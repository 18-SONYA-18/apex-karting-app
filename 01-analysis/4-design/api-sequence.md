...
`slot_id`, `seats_count` (1..4) и `rental_count` (0..seats_count). Итоговую цену `price_total`
...
sequenceDiagram
    ...
    Note over API: Атомарно: проверка свободных мест/<br/>экипировки, фиксация цены, списание (NFR-8/9)
    ...
    alt Успех
        ...
        App-->>User: «Заезд отменён», запись недоступна
...
| `409` | Нет мест/экипировки или двойная бронь; тело несёт `available_seats`/`available_rental_equipment` (Error.details) | common/models.yaml |
...
(места и прокатная экипировка возвращаются в слот), `< 2 ч` → `late_cancel` (не возвращаются, штрафов
...
> в [data-model.md §«Модель состояний»](data-model#модель-состояний-жизненный-цикл).
