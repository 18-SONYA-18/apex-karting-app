...
| name | string | Название (например, «Длинная трасса», «Короткая трасса») |
...
| free_rental_equipment | int | Свободно прокатных комплектов экипировки (из общего фонда центра) |
| price | money (RUB) | Цена за место (валюта — рубли) |
| rental_price | money (RUB) | **Отдельный тариф проката** за один комплект прокатной экипировки (отдельно от `price`); своя экипировка бесплатна. Итог брони = `price × seats_count + rental_price × rental_count`. Валюта — рубли (R-010) |
...
| seats_count | int | Число мест в записи (1–4: себя + гости). Только агрегат, без сущности BookingSeat (R-013) |
| rental_count | int | Сколько из мест — с прокатной экипировкой (0..seats_count). Агрегат, без BookingSeat (R-013) |
| status | enum (`active`/`cancelled`/`late_cancel`/`center_cancelled`) | Статус записи. `center_cancelled` — **«Отменён центром»**: возникает, когда слот отменён центром (`Slot.status = cancelled`); клиент видит бронь как отменённую не по своей инициативе (R-008) |
...
> задаёт пин и текст места сбора (см. [SCR-003](../3-design-brief/SCR-003-slot-card.md),
> [SCR-006](../3-design-brief/SCR-006-booking-details.md) и
> [foundations §4.5](../3-design-brief/00-foundations.md#45-карта-трассы-статичный-превью-яндекскарт)).
...
`status ∈ {active, cancelled, late_cancel, center_cancelled}`. Создаётся в `active`; отмена —
терминальный переход (повторная отмена не выполняется, [UC-2 E2](../2-requirements/use-cases)).
...
`center_cancelled` — **«Отменён центром»** (отмена не по инициативе клиента, R-008); места/экипировка в
...
flowchart LR
    ...
    active -->|слот отменён центром| center_cancelled
    cancelled --> End((◉))
    late_cancel --> End
...
| :-- | :-- | :-- | :-- | :-- |
| — | Клиент подтверждает бронь | `active` | `free_seats −= seats_count`; `free_rental_equipment −= rental_count` | UC-1, FR-10 |
| `active` | Отмена, до старта `≥ 2 ч` | `cancelled` | Места и прокатная экипировка **возвращаются** в слот | UC-2, FR-17 |
| `active` | Отмена, до старта `< 2 ч` | `late_cancel` | Место и прокатная экипировка **НЕ освобождаются**, штрафов нет | UC-2 A1, FR-18 |
| `active` | Слот отменён центром (`Slot.status → cancelled`) | `center_cancelled` | Слот снят; клиент уведомляется (push), запись закрыта не по своей инициативе | R-008, FR-33 |
| `cancelled` / `late_cancel` / `center_cancelled` | — (терминальные) | — | Повторная отмена не выполняется | UC-2 E2 |
...
клиента, [FR-33](../2-requirements/functional-requirements)). Клиент видит статус и реагирует
на UI: при `cancelled` запись недоступна ([SCR-003 §7](../3-design-brief/SCR-003-slot-card.md)).

```mermaid
flowchart LR
    Start((●)) --> scheduled
    scheduled -->|отменён маршалом/центром| cancelled
    cancelled --> End((◉))
```
...
- `Slot.free_rental_equipment = исходный прокатный фонд слота − Σ(active+late_cancel bookings.rental_count)` — прокатная экипировка при поздней отмене тоже НЕ освобождается (комплект простаивает, см. домен); общий прокатный фонд центра — комплекты экипировки.
- Только **ранняя** отмена возвращает места и прокатную экипировку в слот (`cancelled`); `late_cancel` удерживает и место, и комплект экипировки (FR-17/FR-18).
