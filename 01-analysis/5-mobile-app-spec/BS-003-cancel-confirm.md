...
**Зона авторизации:** АЗ  
**Дизайн-макет:** Figma — [Отмена записи (71:13106)](https://www.figma.com/design/ySEt0cjmRqmhdWyDlTpDM5/Апекс-приложение?node-id=71-13106) · [Cancellation Sheet (71:3609)](https://www.figma.com/design/ySEt0cjmRqmhdWyDlTpDM5/Апекс-приложение?node-id=71-3609)
...
> Как клиент, я хочу отменить свою запись до старта заезда, понимая последствия отмены,
> чтобы освободить место, если планы изменились, и не столкнуться со скрытыми санкциями.

### Бизнес-ценность

- При ранней отмене (≥ 2 ч до старта) место и прокатные комплекты экипировки возвращаются в слот и становятся доступны другим клиентам (FR-17).
...
> Параметры брони (конфигурация трассы, места, цена) в шторке **не повторяются** — контекст уже виден на SCR-006. Шторка фокусируется на последствиях и подтверждении (решение брифа §11.3).
...
| **Успех 200** | `status = cancelled` (ранняя) | Закрыть шторку → возврат на SCR-006 с обновлённым статусом «Отменена»; места и прокатная экипировка возвращены в слот. Снек-итог «Бронь отменена» показывает **экран-родитель SCR-006** после закрытия шторки ([00-foundations §6.1/§6.2](../3-design-brief/00-foundations.md)) |
...
|----|----------|-----------|
| FR-16 | Отмена записи клиентом до старта заезда | Must |
| FR-17 | Ранняя отмена (≥ 2 ч): освобождение мест и прокатной экипировки обратно в слот | Must |
...
| AC-002 | **Дано** показана ранняя отмена, **Когда** клиент тапает «Подтвердить отмену» и `cancelBooking` возвращает 200 со `status = cancelled`, **Тогда** места и прокатная экипировка освобождены в слот, шторка закрыта и клиент возвращён на SCR-006 с обновлённым статусом «Отменена». | P0 |
