...
**ID:** SCR-006  
**Тип:** Экран  
**Домен:** 03. Мои записи и отмены  
**Приоритет:** Critical  
**Статус:** Черновик  
**Функциональные блоки:** FB-BOOKINGS-001 (просмотр деталей брони), FB-BOOKINGS-002 (отмена брони)  
**Зона авторизации:** АЗ  
**Дизайн-макет:** [Figma — Детали записи (71:6100)](https://www.figma.com/design/ySEt0cjmRqmhdWyDlTpDM5/Апекс-приложение?node-id=71-6100)
...
Вложенный экран авторизованной зоны. Показывает **полную информацию об одной брони** клиента и служит **точкой запуска её отмены**. Открывается из списка [«Мои записи» (SCR-005)](SCR-005-my-bookings.md) тапом по карточке записи.

Две задачи экрана:
1. **Просмотр.** Собрать в одном месте все параметры брони и её текущий статус: на что и когда записан клиент, конфигурация трассы и её тип, маршал, сколько мест и какая экипировка, итоговую цену (оплата офлайн) и служебные даты.
...
- Самообслуживание при отмене — снимает нагрузку с центра (заменяет переписку в мессенджере).
- Ранняя отмена возвращает места в слот (FR-17) — их можно перезанять, выше заполняемость заездов.
...
|----------|---------|---------|------------------------|
| [SCR-005 Мои записи](SCR-005-my-bookings.md) | Тап по карточке записи | Всегда | `bookingId` |
...
| [BS-004 Карта трассы](BS-004-track-map.md) | Тап по карте трассы / «Открыть карту» | `bookingId` |
| [SCR-005 Мои записи](SCR-005-my-bookings.md) | Тап «Назад» в хедере | — |
...
| [LOGIC-006 Карта трассы](09_Логики/LOGIC-006_Карта-трассы.md) | Блок «Карта трассы + место сбора» | Статичный превью Яндекс-карты с линией трассы и пином; состояния загрузки/ошибки и текстовый fallback; переход в [BS-004](BS-004-track-map.md). |
...
> При открытии экрана отправляется один критичный запрос `getBooking`. Карта трассы рендерится из уже полученных полей вложенного `slot.track_config` (отдельного API-вызова не требует, см. [LOGIC-006](09_Логики/LOGIC-006_Карта-трассы.md)).
...
flowchart LR
    ...
    Check -->|Нет| Err([Error])
    Ready --> Map[Рендер статичной карты из slot.geometry]
```
...
**Ответ:** схема `Booking` (`../api/bookings/models.yaml` → `Booking`) с вложенным `slot` (`../api/slots/models.yaml` → `Slot`), внутри которого `track_config` и `marshal`.
...
│                               │
│  Трасса                       │
│   Длинная трасса · опытная    │  ← slot.track_config.name + track_config.type
│   Маршал: Анна                │  ← slot.marshal.name
│                               │
...
│                               │
│  Цена                         │
│   5300 ₽                      │  ← Booking.price_total (серверный итог)
│   Оплата на месте: наличные   │
│   или перевод на карту.       │
│                               │
│  Записано: 1 июля, 15:20      │  ← created_at
│  (Отменено: 5 июля, 17:00)    │  ← cancelled_at (если есть)
├───────────────────────────────┤
│  Бесплатно освободить место   │  ← подсказка дедлайна (если CTA активна)
│  можно до 12 июля, 16:00      │     start_at − 2 ч (LOGIC-004)
...
| Бейдж статуса | Иконка + текст статуса (Активна / Отменена / Поздняя отмена / Отменена центром / Прошедшая). | Да |
| Блок «Когда» | Дата и время старта (`slot.start_at`), крупно. | Да |
| Блок «Трасса и маршал» | Название и тип конфигурации трассы, имя маршала. | Да |
| Карта трассы + место сбора | Статичный превью карты + текст места сбора. | Да |
| Блок «Места и экипировка» | Число мест и разбивка прокатные/свои. | Да |
| Блок «Цена» | Итоговая сумма + текст офлайн-оплаты. | Да |
| Служебные метки | Дата создания; дата отмены — только если бронь отменена; причина отмены центром (`cancellation_reason`) — при `center_cancelled`. | Да (создана) / Опционально (отменена) |
...
- Статус определяется по `status` и времени старта (см. [LOGIC-004](09_Логики/LOGIC-004_Отмена-правило-2-часов.md), [LOGIC-008](09_Логики/LOGIC-008_Паттерн-состояний-экрана.md)):
  ...
  - **Отменена центром** — `status = center_cancelled` (бронь отменил центр/маршал; места освобождены). Под бейджем показывается текст причины отмены из `cancellation_reason` (если заполнен).
...
| Элемент | Описание | Источник данных | Валидация | Действие |
|---------|----------|-----------------|-----------|----------|
| Дата и время старта | Дата и время начала заезда, крупно | `slot.start_at` из [getBooking](#getbooking) | — | — |

### 4. Блок «Трасса и маршал»

| Элемент | Описание | Источник данных | Валидация | Действие |
|---------|----------|-----------------|-----------|----------|
| Название конфигурации трассы | Имя конфигурации трассы | `slot.track_config.name` | — | — |
| Тип конфигурации трассы | Новичковая / опытная | `slot.track_config.type` (`novice` / `experienced`) | — | — |
| Маршал | Имя ведущего заезд | `slot.marshal.name` | — | — |

### 5. Карта трассы + место сбора

| Элемент | Описание | Источник данных | Валидация | Действие |
|---------|----------|-----------------|-----------|----------|
| Статичная карта | Превью Яндекс-карты с линией трассы и пином места сбора | `slot.geometry` + `slot.meeting_point_lat`/`meeting_point_lng` | — | Открыть [BS-004](BS-004-track-map.md) |
| «Открыть карту ›» | Ссылка/кнопка перехода к интерактивной карте | — | — | Открыть [BS-004](BS-004-track-map.md) |
| Место сбора (текст) | Адрес/ориентир под картой (обязателен) | `slot.meeting_point` | — | — |

**Логика:**
- Карта трассы: [LOGIC-006](09_Логики/LOGIC-006_Карта-трассы.md) — статичный превью; при загрузке — скелетон в форме карты; при ошибке/офлайн/отсутствии ключа — fallback на текстовый блок места сбора + ссылку «Открыть в Яндекс.Картах». Остальные детали брони при ошибке карты остаются доступны.
- Текстовый блок места сбора обязателен и служит текстовым эквивалентом карты (доступность).

### 6. Блок «Места и экипировка»

| Элемент | Описание | Источник данных | Валидация | Действие |
|---------|----------|-----------------|-----------|----------|
| Число мест | Сколько мест в брони (себя + гости) | `seats_count` | — | — |
| Разбивка экипировки | Сколько прокатных / своих | `rental_count` прокатных; `seats_count − rental_count` своих | — | — |

**Логика:**
- Лейблы «Прокатная экипировка» / «Своя экипировка» — из [foundations §6](../3-design-brief/00-foundations.md#6-tone-of-voice-и-общая-микрокопия). Числа не хардкодятся, считаются из `seats_count` и `rental_count`.
...
| Причина отмены центром | Текст причины (только при `status = center_cancelled`) | `cancellation_reason` | — | — |

**Логика:**
- Метка «Отменено» отображается **только** если `cancelled_at != null` (для статусов `cancelled` / `late_cancel` / `center_cancelled`).
- Для `status = center_cancelled` дополнительно показывается текст причины отмены из `cancellation_reason` (если заполнен) — рядом с бейджем «Отменена центром» / в блоке деталей отмены.
...
**Условия доступности:**
- Кнопка «Отменить» **активна (enabled)**, только если: `status = active` **И** `slot.start_at` в будущем (заезд не начался).
- Кнопка **неактивна (disabled, не скрывается)** с обязательным пояснением, если:
  - `slot.start_at` уже наступил (статус «Прошедшая», UC-2 E1) → «Заезд уже начался — отменить запись нельзя.»
  - `status = cancelled`, `status = late_cancel` или `status = center_cancelled` (UC-2 E2, повторная отмена не выполняется) → «Запись уже отменена.»
...
| Content (Отменена центром) | 200 + `status = center_cancelled` | Полные детали; бейдж «Отменена центром» + текст причины `cancellation_reason`; показан `cancelled_at`; CTA disabled «Запись уже отменена» (UC-2 E2) |
| MapError | Ошибка/офлайн карты при успешном `getBooking` | Карта → текстовый fallback (место сбора + ссылка); остальные детали доступны (LOGIC-006) |
...
stateDiagram-v2
    ...
    Loading --> Cancelled : 200 OK (cancelled / late_cancel / center_cancelled)
...
| Открыть карту | Карта трассы / «Открыть карту ›» | Tap | Открыть [BS-004](BS-004-track-map.md) (`bookingId`) |
...
|----|----------|-----------|
| FR-16 | Отмена своей записи до старта заезда | Must |
...
| FR-35a | Показ брони со статусом, параметрами слота, числом мест и вариантом экипировки | Must |
...
| ID | Название | Связь |
|----|----------|-------|
| [UC-2](../2-requirements/use-cases) | Отмена записи (осн. поток + A1 поздняя; E1 после старта, E2 повторная) | Запуск отмены, доступность CTA |
| [US-10](../2-requirements/user-stories) | Клиент видит детали брони и отменяет её до старта | Назначение экрана |
...
|----|----------|-----------|
| AC-001 | **Дано** бронь клиента в статусе `active` со стартом в будущем, **Когда** экран в Content, **Тогда** отображаются статус, дата/время старта, конфигурация трассы и тип, маршал, число мест, разбивка экипировки, итоговая цена, текст офлайн-оплаты и дата создания | P0 |
| AC-002 | **Дано** успешный ответ `getBooking`, **Когда** экран в Content, **Тогда** показан статичный превью карты с линией трассы и пином, а под картой — текстовый блок места сбора (`slot.meeting_point`) | P0 |
...
| AC-005 | **Дано** бронь со статусом `cancelled`, `late_cancel` или `center_cancelled`, **Когда** экран в Content, **Тогда** отображается дата отмены (`cancelled_at`) | P0 |
| AC-011 | **Дано** бронь со статусом `center_cancelled` (отменена центром), **Когда** экран в Content, **Тогда** показан бейдж «Отменена центром» и текст причины из `cancellation_reason` (если заполнен), а кнопка «Отменить» disabled с пояснением «Запись уже отменена» | P1 |
| AC-006 | **Дано** тап по карте или «Открыть карту», **Когда** карта доступна, **Тогда** открывается [BS-004](BS-004-track-map.md) с интерактивной картой и трассой | P1 |
...
| AC-N02 | **Дано** `status = active`, но `slot.start_at` уже наступил (заезд начался/прошёл, UC-2 E1), **Когда** экран в Content, **Тогда** кнопка «Отменить» disabled с пояснением «Заезд уже начался — отменить запись нельзя» | P0 |
| AC-N03 | **Дано** бронь уже отменена (`cancelled` / `late_cancel` / `center_cancelled`, UC-2 E2), **Когда** экран в Content, **Тогда** кнопка «Отменить» disabled с пояснением «Запись уже отменена» (повторная отмена недоступна) | P0 |
| AC-N04 | **Дано** карта недоступна (ошибка/офлайн/нет ключа), **Когда** экран в Content, **Тогда** карта заменяется текстовым fallback (место сбора + ссылка «Открыть в Яндекс.Картах»), остальные детали остаются доступны | P1 |
| AC-N05 | **Дано** `cancelBooking` в [BS-003](BS-003-cancel-confirm.md) вернул **422 `slot_started`** (слот стартовал между открытием экрана и подтверждением), **Когда** управление возвращается на SCR-006, **Тогда** экран перечитывает бронь (`getBooking`), статус **не меняется** самопроизвольно, кнопка «Отменить» становится disabled с пояснением «Заезд уже начался», а итог-снек «Слот уже стартовал — отмена недоступна.» синхронизирован с [BS-003](BS-003-cancel-confirm.md) (не дублируется) | P1 |
...
| AC-E02 | **Дано** до старта осталось менее 2 часов, но заезд ещё не начался, **Когда** экран в Content, **Тогда** кнопка «Отменить» активна (доступна до старта), а отмена в [BS-003](BS-003-cancel-confirm.md) будет поздней | P1 |
...
| AC-E04 | **Дано** все места брони — свои (`rental_count = 0`), **Когда** экран в Content, **Тогда** в блоке «Места и экипировка» показано число мест и «своя экипировка» без прокатных, числа считаются из `seats_count`/`rental_count` | P2 |
