...
**Зона авторизации:** АЗ  
**Дизайн-макет:** [Figma — Профиль (71:6677)](https://www.figma.com/design/ySEt0cjmRqmhdWyDlTpDM5/Апекс-приложение?node-id=71-6677) · [Изменение профиля (71:6690)](https://www.figma.com/design/ySEt0cjmRqmhdWyDlTpDM5/Апекс-приложение?node-id=71-6690)
...
- **Удаление аккаунта** (деструктивно, ПДн) → `deleteAccount` (DELETE `/profile`) с обязательным явным подтверждением → [SCR-001](SCR-001-registration.md). **Последствия удаления (R-009):** активные брони (`status = active`) **аннулируются и освобождают места и прокатную экипировку** в своих слотах; прошедшие/завершённые брони **анонимизируются** (обезличиваются от ПДн), но остаются в данных центра для статистики.
- **Справочные пункты** — «Правила центра», «Поддержка» (URL из конфигурации), «Версия приложения» (текст из сборки).
...
> Как клиент картинг-центра, я хочу видеть и при необходимости менять своё имя и номер телефона, а также безопасно выходить из аккаунта,
...
- Прозрачность и доверие: правила центра, контакты поддержки и версия приложения доступны из одного места.
...
| [SCR-002 Список слотов](SCR-002-slot-list.md) | Тап на таб «Заезды» | — |
| [SCR-005 Мои записи](SCR-005-my-bookings.md) | Тап на таб «Мои записи» | — |
| Правила центра (внешний ресурс) | Тап на «Правила центра ›» | URL из конфигурации |
...
| `rulesUrl` | Remote Config / конфигурация | URL-строка | Адрес страницы «Правила центра». |
...
│                                     │
├─────────────────────────────────────┤
│ Заезды · Мои записи · ●Профиль       │  ← Tab-bar (foundations §4.2)
└─────────────────────────────────────┘
```

### Просмотр + инлайн-карандаши (макет [71:6677](https://www.figma.com/design/ySEt0cjmRqmhdWyDlTpDM5/Апекс-приложение?node-id=71-6677))
...
│  · · · · · · · · · · · · ·   │
│  Правила центра           ›  │  ← справочные пункты
│  Поддержка                ›  │
...
### Инлайн-правка одного поля (макет [71:6690](https://www.figma.com/design/ySEt0cjmRqmhdWyDlTpDM5/Апекс-приложение?node-id=71-6690))
...
| Справочные пункты | «Правила центра ›», «Поддержка ›» (переходы), «Версия приложения» (текст) | Да |
...
| Элемент | Описание | Источник данных | Валидация | Действие |
|---------|----------|-----------------|-----------|----------|
| «Правила центра ›» | Строка-переход | `rulesUrl` из конфигурации | — | Открыть внешний ресурс по `rulesUrl` |
...
- Заголовок «Выйти из аккаунта?»; пояснение «Чтобы снова записаться на заезд, нужно будет войти по номеру телефона.»
...
| Открыть правила центра | «Правила центра ›» | Tap | Внешний ресурс по `rulesUrl` |
| Открыть поддержку | «Поддержка ›» | Tap | Внешний ресурс/контакт по `supportUrl` |
| Перейти к заездам | Таб «Заезды» | Tap | [SCR-002](SCR-002-slot-list.md) |
...
| AC-006 | **Дано** клиент на экране «Профиль», **Когда** он нажимает «Удалить аккаунт» и подтверждает деструктивное действие, **Тогда** отправляется `deleteAccount`, аккаунт и ПДн удаляются, **активные брони аннулируются и освобождают места/прокатную экипировку**, прошедшие **анонимизируются** (R-009), сессия завершается и он попадает на [SCR-001](SCR-001-registration.md). | P0 |
...
| AC-009 | **Дано** клиент тапает «Правила центра» или «Поддержка», **Когда** ссылка задана в конфигурации, **Тогда** открывается соответствующий внешний ресурс; версия приложения отображается как нередактируемый текст. | P1 |
