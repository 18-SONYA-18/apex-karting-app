...
**Зона авторизации:** АЗ  
**Дизайн-макет:** [Figma — Запись подтверждена (71:6161)](https://www.figma.com/design/ySEt0cjmRqmhdWyDlTpDM5/Апекс-приложение?node-id=71-6161)
...
заездов. По дизайну ([RR-D03](../3-design-brief/design-review.md)) это **полноэкранный** успех
...
прокатной экипировки, гонка запросов, сетевой сбой) обрабатываются **на SCR-004** и до этого экрана не
...
> сводку брони (когда, какая трасса, маршал, сколько мест и экипировки, сколько платить и как),
...
| [SCR-002 «Заезды»](SCR-002-slot-list.md) | Тап «Готово» (secondary) | — (стек оформления записи завершён; SCR-003/SCR-004 в стеке не остаются) |
...
> вкладку «Заезды» [SCR-002](SCR-002-slot-list.md); «Мои бронирования» ведёт в
...
> Все значения сводки (время старта, конфигурация трассы, маршал, число мест, разбивка экипировки, итоговая
> цена) подставляются из объекта `booking` — **без хардкода** (foundations §6). Лейблы экипировки
> «Своя экипировка» / «Прокатная экипировка» и текст офлайн-оплаты — из foundations §6.
...
|--------|-----------------|----------|
| [LOGIC-003 Расчёт цены брони](09_Логики/LOGIC-003_Расчёт-цены-брони.md) | Блок «Итого» | Итоговая цена брони — **серверное поле `price_total`** ответа `createBooking` (R-005). На BS-002 показывается **`booking.price_total`**, клиент его **не пересчитывает**. Тарифы `price` / `rental_price` лежат в `booking.slot` и нужны лишь для разбивки «своих / прокатных» комплектов экипировки. |
...
> — это обращение к системному API ОС, а не к REST API «Апекса».
...
|------|-----|---------------------|
| `slot.start_at` | string (date-time) | Дата и время старта заезда |
| `slot.track_config.name` | string | Название конфигурации трассы |
| `slot.marshal.name` | string | Имя маршала |
| `seats_count` | integer `1…4` | Число забронированных мест (себя + гости) |
| `rental_count` | integer `0…seats_count` | Число прокатных комплектов экипировки; своих комплектов = `seats_count − rental_count` |
...
| `slot.rental_price` | integer (RUB) | Тариф проката за один комплект экипировки (в `booking.slot`, R-005); для разбивки |
...
│  │ 🏁  Трасса: <track_config.name>  │ │  ← сводка брони
│  │ 👤  Маршал: <marshal.name>       │ │
│  │ 👥  Мест: <seats_count>          │ │
│  │ 🥽  Экипировка: своих <K> · прокат <M>│ │  ← K = seats−rental, M = rental_count
│  │ ───────────────────────────────  │ │
...
│  └─────────────────────────────────┘ │
│  ┌─────────────────────────────────┐ │
│  │           Готово                │ │  ← secondary → SCR-002 «Заезды»
│  └─────────────────────────────────┘ │
└─────────────────────────────────────┘
```

> Полноэкранный успех (макет [71:6161](https://www.figma.com/design/ySEt0cjmRqmhdWyDlTpDM5/Апекс-приложение?node-id=71-6161)):
...
| Сводка брони | Карточка «лейбл → значение»: старт, конфигурация трассы, маршал, число мест, разбивка экипировки, итог цены. | Да |
...
| Кнопка «Готово» | Secondary CTA во всю ширину → [SCR-002](SCR-002-slot-list.md) «Заезды». | Да |
...
| Элемент | Описание | Источник данных | Валидация | Действие |
|---------|----------|-----------------|-----------|----------|
| Дата/время старта | «Сб, 21 июн · 18:00» | `booking.slot.start_at` | — | — |
| Конфигурация трассы | «Трасса: …» | `booking.slot.track_config.name` | — | — |
| Маршал | «Маршал: …» | `booking.slot.marshal.name` | — | — |
| Число мест | «Мест: N» | `booking.seats_count` | — | — |
| Разбивка экипировки | «своих K · прокат M», где `K = seats_count − rental_count`, `M = rental_count` | `booking.seats_count`, `booking.rental_count` | — | — |
| Итог цены | «Итого: … ₽» (крупно, контрастно) — **серверное поле** | `booking.price_total` | — | — |

**Логика:**
- Разбивка экипировки: лейблы «Своя экипировка» / «Прокатная экипировка» — foundations §6. При `rental_count = 0`
  строка про прокат может скрываться (за дизайнером), при этом факт «своя экипировка» остаётся понятен.
- Итог цены: [LOGIC-003](09_Логики/LOGIC-003_Расчёт-цены-брони.md) — показывается **серверное поле
  ...
  `booking.slot.price` / `booking.slot.rental_price` нужны лишь для разбивки экипировки.
...
| Кнопка «Готово» | Secondary CTA во всю ширину | — | — | Уход с экрана → [SCR-002](SCR-002-slot-list.md) «Заезды» |
...
- «Готово»: при тапе — уход с экрана успеха и возврат на корневую вкладку «Заезды»
...
| Завершить | Кнопка «Готово» | Tap | Уход с экрана → [SCR-002](SCR-002-slot-list.md) «Заезды» |
...
| ID | Название |
|----|----------|
| UC-1 | Запись на заезд — завершающий (успешный) шаг: SCR-002 → SCR-003 → SCR-004 → **BS-002** |
| US-5 | Записаться на слот и получить подтверждение записи |
| US-11 | Видеть цену заезда; оплата офлайн |
...
|----|----------|-----------|
| AC-001 | **Дано** клиент подтвердил запись на SCR-004 и бронь успешно создана (201), **Когда** открывается BS-002, **Тогда** виден блок успеха (иконка + «Вы записаны») и сводка брони: дата/время старта, конфигурация трассы, маршал, число мест, разбивка экипировки и итог цены — все значения из объекта `booking`, без хардкода. | P0 |
...
| AC-004 | **Дано** в брони есть прокатные места (`rental_count > 0`), **Когда** открыта BS-002, **Тогда** сводка показывает соотношение своих (`seats_count − rental_count`) и прокатных (`rental_count`) комплектов экипировки. | P1 |
...
| AC-N03 | **Дано** открыт экран BS-002, **Когда** клиент нажимает «Готово» (secondary), **Тогда** клиент уходит на список «Заезды» [SCR-002](SCR-002-slot-list.md). | P0 |

### Граничные условия (Edge Cases)

| ID | Критерий | Приоритет |
|----|----------|-----------|
| AC-E01 | **Дано** открыт экран BS-002, **Когда** клиент нажимает аппаратный/системный «назад», **Тогда** исход тот же, что и «Готово» — возврат на [SCR-002](SCR-002-slot-list.md) «Заезды»; свободного закрытия свайпом/бэкдропом у экрана нет, бронь уже сохранена. | P0 |
| AC-E02 | **Дано** `rental_count = 0` (вся экипировка своя), **Когда** открыт BS-002, **Тогда** строка про прокат может быть скрыта, но факт «своя экипировка» в сводке остаётся понятен. | P2 |
| AC-E03 | **Дано** максимум мест (`seats_count = 4`) и длинная сводка, **Когда** контент не помещается, **Тогда** содержимое скроллится внутри экрана, а кнопки «Мои бронирования» и «Готово» остаются достижимыми. | P2 |
