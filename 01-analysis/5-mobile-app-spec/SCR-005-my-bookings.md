...
**Зона авторизации:** АЗ  
**Дизайн-макет:** [Figma — Мои записи (71:6085)](https://www.figma.com/design/ySEt0cjmRqmhdWyDlTpDM5/Апекс-приложение?node-id=71-6085)
...
> и название файла («Мои записи») сохранены — правится только видимая микрокопия.
...
|-------|-----|-------------------|
| 0.1.0 | SCR-005 «Мои записи» | Первичная версия ТЗ на основе [дизайн-брифа SCR-005](../3-design-brief/SCR-005-my-bookings.md). |
...
Контекст использования — трасса, яркое солнце, спешка перед заездом: клиенту нужно **с одного взгляда** понять ближайшую запись (когда старт, какая конфигурация трассы, сколько мест) и её статус. Карточка — сводка для опознания записи; полная детализация — на [SCR-006](SCR-006-booking-details.md).

Данные загружаются через `listBookings` (`GET /bookings`) и содержат только брони текущего клиента (NFR-12). Показывается **вся история** броней с **пагинацией** (`limit`/`offset`, ленивая догрузка по мере скролла, R-025). Сервер сортирует по `slot.start_at` (по убыванию); разделение на предстоящие/прошедшие и признак «прошедшее» **вычисляются на клиенте** по `slot.start_at` и `status` — отдельного флага сервер не отдаёт. Отменённые, поздно отменённые и отменённые центром (`cancelled` / `late_cancel` / `center_cancelled`) брони из истории **не исключаются** — видны с соответствующим бейджем; для `center_cancelled` показывается текст причины отмены `cancellation_reason`.
...
- Самообслуживание: вход в детали и отмену без обращения к центру.
...
| [SCR-002 Список слотов](SCR-002-slot-list.md) | Тап «Записаться на заезд» в Empty-состоянии | — |
| [SCR-002 Список слотов](SCR-002-slot-list.md) | Тап по табу «Заезды» | — |
...
|----------|-----|----------------|----------|----------|
| `status` | string (`active`/`cancelled`/`late_cancel`/`center_cancelled`) | Нет | — | Опциональный фильтр по статусу. На SCR-005 **не передаётся**: грузим **всю историю** (включая отменённые/late_cancel/center_cancelled), разделение делаем на клиенте. |
...
**Ответ (200):** `BookingListResponse` = `items: BookingSummary[]` + `meta` (пагинация). Поля `BookingSummary`, используемые на экране: `id`, `seats_count`, `rental_count`, **`price_total`** (серверный итог, RUB), `status` (`active`/`cancelled`/`late_cancel`/`center_cancelled`), `cancelled_at`, `cancellation_reason` (причина отмены центром, заполняется при `status = center_cancelled`), `slot` (`SlotSummary` → `start_at`, `track_config.name`, `track_config.type`, `marshal.name`, тарифы `price`/`rental_price` для разбивки).
...
- **Прошедшие** — `slot.start_at <= now` **ИЛИ** `status ∈ {cancelled, late_cancel, center_cancelled}` (отменённая запись — в т.ч. отменённая центром — с будущим стартом тоже попадает в «Прошедшие»).
...
> Признак «прошедшая» — производный (вычисляется из `slot.start_at`), а не статус брони. Параметр `status` запроса в API **не передаётся** (см. [listBookings](#listbookings)); разделение на табы выполняется на клиенте по `slot.start_at` и `Booking.status` (включая `center_cancelled`).
...
│ │ сб, 21 июн · 18:00       │ │
│ │ Длинная трасса · Опытная │ │
│ │ Маршал: Анна             │ │
│ │ 2 места · 1 прокатная    │ │
│ │ 5300 ₽          [активна]│ │  ← бейдж статуса (текст+форма)
│ └──────────────────────────┘ │
│ ┌──────────────────────────┐ │
│ │ вс, 22 июн · 19:00       │ │
│ │ Короткая трасса · Новичковая│ │
│ │ Маршал: Игорь            │ │
...
| Конфигурация трассы | Название + тип конфигурации трассы, если приходит | `slot.track_config.name` (+ `slot.track_config.type`: `novice`→«новичковая», `experienced`→«опытная») | — | — |
...
| Бейдж статуса | Текст + форма: `active`→«активна», `cancelled`→«отменена», `late_cancel`→«поздняя отмена», `center_cancelled`→«отменена центром» | `status` | — | — |
...
- Для `center_cancelled` («Отменена центром») под бейджем/в карточке показывается текст причины из `cancellation_reason` (если заполнен).
...
| Кнопка «Записаться на заезд» | Единый CTA во всех Empty-состояниях экрана | — | — | Открыть [SCR-002](SCR-002-slot-list.md) |
...
- Единый CTA для всех Empty-состояний — «Записаться на заезд» → [SCR-002](SCR-002-slot-list.md).
...
| Записаться | Кнопка «Записаться на заезд» | Tap (в Empty) | Переход на [SCR-002](SCR-002-slot-list.md) |
| Перейти в «Заезды» | Таб «Заезды» | Tap | Переход на [SCR-002](SCR-002-slot-list.md) |
...
|----|----------|-----------|
| FR-35a | Показывать клиенту список его бронирований (предстоящие и прошедшие) со статусом, параметрами слота, числом мест и вариантом экипировки — [Функциональные требования](../2-requirements/functional-requirements) | Must |
...
| ID | Название | Приоритет |
|----|----------|-----------|
| US-9 | Клиент видит список своих записей (предстоящих и прошедших) со статусом, параметрами слота, числом мест и вариантом экипировки — [User stories](../2-requirements/user-stories) | Must |

### Данные (REQ-DATA-*)

| ID | Название | Приоритет |
|----|----------|-----------|
| NFR-12 | Клиент видит и управляет только своими записями; чужие/административные данные недоступны — [Нефункциональные требования](../2-requirements/non-functional-requirements) | Высокий |
...
| AC-004 | **Дано** бронь имеет статус `active`/`cancelled`/`late_cancel`/`center_cancelled`, **Когда** клиент видит карточку, **Тогда** на ней бейдж «активна»/«отменена»/«поздняя отмена»/«отменена центром», переданный текстом и формой, а не только цветом; для `center_cancelled` дополнительно показан текст причины `cancellation_reason` | P0 |
...
| AC-007 | **Дано** на карточке `rental_count = 0`, **Когда** клиент её видит, **Тогда** показана «своя экипировка»; **Дано** `rental_count > 0`, **Тогда** показано число прокатных комплектов экипировки | P1 |
...
| AC-009 | **Дано** в истории есть отменённые/поздно отменённые/отменённые центром брони (`cancelled`/`late_cancel`/`center_cancelled`), **Когда** клиент открывает экран без фильтра по статусу, **Тогда** они присутствуют в истории («Прошедшие») с соответствующим бейджем (для `center_cancelled` — «отменена центром» + причина `cancellation_reason`) — из выдачи не исключаются (R-025) | P1 |
...
| AC-N02 | **Дано** у клиента нет ни одной брони (`items` пуст), **Когда** он открывает экран, **Тогда** показана заглушка «У вас пока нет записей» с действием «Записаться на заезд» → [SCR-002](SCR-002-slot-list.md) | P0 |
...
| ID | Критерий | Приоритет |
|----|----------|-----------|
| AC-E01 | **Дано** бронь отменена (`cancelled`/`late_cancel`/`center_cancelled`), но `slot.start_at` ещё в будущем, **Когда** клиент открывает экран, **Тогда** она показана в «Прошедших», а не в «Предстоящих» | P0 |
