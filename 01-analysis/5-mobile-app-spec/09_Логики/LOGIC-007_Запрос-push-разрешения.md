...
> обеспечивает существующая инфраструктура; приложение через клиентский REST API «Апекса»
...
**регистрирует его в API «Апекса»** запросом `registerPushToken` (`POST /auth/push-tokens`, R-006);
...
> Как клиент, я хочу после первой записи разрешить напоминания о заезде,
> чтобы не забыть и не пропустить старт, — но без навязчивых запросов, если я отказался.

### Бизнес-ценность

- Снижение неявок: заблаговременное напоминание о предстоящем заезде (US-12, NFR-13).
...
(APNs / FCM) и **регистрирует его в API «Апекса»** запросом `registerPushToken`
...
> согласии приложение **регистрирует push-токен в REST API «Апекса»** (`registerPushToken`, R-006);
...
| REQ-INT-PUSH | Push API «Апекса»: `registerPushToken`, `deletePushToken` ([`../../api/auth/api.yaml`](../../api/auth/api.yaml)), R-006 | Medium |

### UI (REQ-UI-*)

| ID | Название | Приоритет |
|----|----------|-----------|
| US-12 | Получать напоминание о предстоящей записи, чтобы не забыть и не пропустить заезд | Medium (Should) |
