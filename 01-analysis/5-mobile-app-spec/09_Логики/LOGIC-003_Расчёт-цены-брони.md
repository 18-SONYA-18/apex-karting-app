...
| `slot.rental_price` | Данные слота (`getSlot` / `Slot.rental_price` / `SlotSummary.rental_price`) | целое ≥ 0, RUB | Тариф проката за **один прокатный комплект экипировки** (за комплект, не за место). Лежит в слоте/`booking.slot` (R-005, R-010). |
| `seats_count` | Состояние формы SCR-004 / поле брони | `1`–`4` | Число мест в брони. |
| `rental_count` | Состояние формы SCR-004 / поле брони | `0`–`4` (≤ `seats_count`) | Число прокатных комплектов экипировки. |
...
Логика отвечает за отображение итоговой стоимости брони заезда во всех точках клиентского сценария. Источник итога зависит от того, создана ли уже бронь:
...
- `slot.price` — тариф за одно место; `slot.rental_price` — отдельный тариф за один прокатный комплект экипировки (R-010); итог зависит и от числа прокатных мест. Оба тарифа лежат в слоте (`booking.slot` для брони, R-005).
- Прокат — **отдельный тариф**, считается **за комплект экипировки, а не за место**: своя экипировка бесплатна (`rental_price` на неё не начисляется).
...
- Снижение барьера к записи — понятная цена и явное правило офлайн-оплаты вместо Telegram/доски маркером.
...
| [SCR-004 Оформление записи](../SCR-004-booking.md) | Блок цены (разбивка + **превью** итога) и сумма на кнопке «Записаться»; пересчёт при изменении мест / выбора экипировки | Всегда (на экране оформления; брони ещё нет → клиентский расчёт) |
...
```mermaid
flowchart TD
    Start([Изменение seats_count / выбора экипировки на SCR-004]) --> GetData[Взять slot.price, slot.rental_price из данных слота]
...
- Стоимость проката = `slot.rental_price × rental_count` (отдельный тариф; начисляется только за прокатные комплекты экипировки — своя экипировка бесплатна, R-010).
...
При любом изменении числа мест (`seats_count`) или варианта экипировки на любом месте (что меняет `rental_count`) итог и все строки разбивки пересчитываются немедленно, без отдельного действия пользователя. Сумма на CTA обновляется синхронно.
...
| AC-003 | **Дано** часть мест с прокатной экипировкой (`rental_count > 0`), **Когда** клиент выбрал экипировку, **Тогда** превью итога = `slot.price × seats_count + slot.rental_price × rental_count`, появляется строка «Прокат: `slot.rental_price` × `rental_count`», и прокат начисляется за комплекты экипировки, а не за все места (своя экипировка бесплатна, R-010). |
