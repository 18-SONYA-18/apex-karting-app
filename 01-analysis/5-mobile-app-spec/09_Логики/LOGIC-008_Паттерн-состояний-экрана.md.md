...
- Доступность у трассы: состояния дублируются текстом/иконкой/формой, считываются на солнце и при дальтонизме (NFR-1).
...
| [SCR-006 Детали записи](../SCR-006-booking-details.md) | При открытии экрана | Пример применения |
...
|----------------|-----------------|--------------------------|----------|
| Нет доступных заездов вообще | «Пока нет доступных заездов» | «Потяните вниз, чтобы обновить» | [SCR-002](../SCR-002-slot-list.md) |
| Нет заездов по выбранным фильтрам | «Ничего не найдено по фильтрам» | CTA «Изменить фильтры» | [SCR-002](../SCR-002-slot-list.md) |
| Нет предстоящих записей | «Пока нет предстоящих записей» | CTA «Записаться на заезд» → [SCR-002](../SCR-002-slot-list.md) | [SCR-005](../SCR-005-my-bookings.md) |
| Нет прошедших записей | «Здесь появятся прошедшие заезды» | CTA «Записаться на заезд» → [SCR-002](../SCR-002-slot-list.md) | [SCR-005](../SCR-005-my-bookings.md) |
| Трасса без геометрии (`slot.geometry = null`) | «Трасса на карте недоступна» (пин места сбора + текст) | — (это Content-без-линии, не Error) | [BS-004](../BS-004-track-map.md), [LOGIC-006](LOGIC-006_Карта-трассы.md) |
...
|----|----------|-----------|
| NFR-1 | Удобство использования: mobile-first, крупные элементы, высокий контраст; состояния дублируются не только цветом (доступность у трассы/при дальтонизме) | High |
