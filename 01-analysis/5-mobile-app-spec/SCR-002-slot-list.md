...
**Зона авторизации:** АЗ  
**Дизайн-макет:** Figma — [Загрузка списка (71:5695)](https://www.figma.com/design/ySEt0cjmRqmhdWyDlTpDM5/Апекс-приложение?node-id=71-5695) · [Нет заездов / Empty (71:6002)](https://www.figma.com/design/ySEt0cjmRqmhdWyDlTpDM5/Апекс-приложение?node-id=71-6002) · [Ошибка загрузки (71:6026)](https://www.figma.com/design/ySEt0cjmRqmhdWyDlTpDM5/Апекс-приложение?node-id=71-6026)
...
**SCR-002 «Список слотов» — главный экран приложения «Апекс» и корневой таб
авторизованной зоны.** Открывается сразу после успешного входа и доступен по табу
«Заезды». Показывает каталог предстоящих заездов картинг-центра и служит единственной
...
конфигурации трассы, маршала, цену и доступность мест («Свободно N из M»). Слот без свободных мест
...
> Как клиент, я хочу видеть список свободных слотов заездов,
...
- Прозрачность доступности: заполненные слоты видны с пометкой «Мест нет», что снижает путаницу и заменяет ручную запись через Telegram/доску маркером.
- Предсказуемость выдачи: единый порядок (ближайшие первыми) и группировка по дням упрощают ориентацию в расписании.
- Воспринимаемая скорость: скелетоны и pull-to-refresh дают ощущение мгновенного отклика у трассы (NFR-6, P4).
...
| `appliedFilters.track_type` | Состояние | массив из `novice`, `experienced`; по умолчанию `[]` | Типы конфигурации трассы (OR внутри группы); пустой массив = любая конфигурация. |
| `appliedFilters.marshal_id` | Состояние | массив `uuid`; по умолчанию `[]` | Маршалы (OR внутри группы); пустой массив = любой маршал. |
...
> Справочник маршалов для фильтра (`listMarshals`) запрашивается не на этом экране,
...
| `track_type` | array(enum) `novice`/`experienced` | Нет | `appliedFilters.track_type` | Типы конфигурации трассы, OR внутри группы; опускается, если массив пуст. |
| `marshal_id` | array(uuid) | Нет | `appliedFilters.marshal_id` | Маршалы, OR внутри группы; опускается, если массив пуст. |
...
Поля `SlotSummary`: `id`, `start_at`, `track_config` (`name`, `type`, `capacity_cap`), `marshal` (`name`),
`price`, `rental_price`, `total_seats`, `free_seats`, `free_rental_equipment`, `status`.
...
| Успех | `items` пуст, фильтры в дефолте | Empty «нет слотов вообще»: заголовок «Пока нет доступных заездов» + подсказка «Загляните позже» (микрокопия дизайна, RR-D11; список обновляется жестом pull-to-refresh) |
...
│  ┌─────────────────────────────────┐ │
│  │ Сб, 21 июня · 18:00             │ │  ← start_at (крупно)
│  │ Длинная трасса · Опытная        │ │  ← track_config.name · track_config.type
│  │ Маршал: Анна                    │ │  ← marshal.name
│  │ ───────────────────────────────  │ │
│  │ 2500 ₽          Свободно 4 из 14│ │  ← price | free_seats из total_seats
│  └─────────────────────────────────┘ │
│  ┌─────────────────────────────────┐ │
│  │ Сб, 21 июня · 19:00             │ │
...
│  │ Маршал: Игорь                   │ │
│  │ ───────────────────────────────  │ │
│  │ 1500 ₽               [ Мест нет ]│ │  ← free_seats = 0 → бейдж, некликабельна
│  └─────────────────────────────────┘ │
...
| Карточка слота | Дата/время старта, конфигурация трассы+тип, маршал, цена, доступность; вся карточка — тап-зона | Да |
| Бейдж «Мест нет» | Текст + иконка/форма (не только цвет) на карточке с `free_seats = 0` | Условно (при `free_seats = 0`) |
| Заглушка Empty «нет слотов вообще» | Заголовок «Пока нет доступных заездов» + подсказка «Загляните позже» (микрокопия дизайна, RR-D11; обновление — жестом pull-to-refresh) | Условно (при пустом `items` и фильтрах в дефолте) |
...
| Название конфигурации трассы | Что за заезд | `track_config.name` из `listSlots` | — | — |
| Тип конфигурации трассы | Лейбл «Новичковая» / «Опытная» (текст, не только цвет) | `track_config.type` из `listSlots` | — | — |
| Маршал | Кто ведёт заезд | `marshal.name` из `listSlots` | — | — |
...
| Элемент | Описание | Источник данных | Валидация | Действие |
|---------|----------|-----------------|-----------|----------|
| Заголовок Empty «нет слотов вообще» | «Пока нет доступных заездов» | — | — | — |
...
| Empty (нет слотов вообще) | 200 + `items` пуст, фильтры в дефолте | Заглушка: «Пока нет доступных заездов» + «Загляните позже» (микрокопия дизайна, RR-D11; обновление — жестом pull-to-refresh) |
...
|----|----------|-----------|
| FR-9 | Показ списка слотов (дата/время, конфигурация трассы, маршал, всего/свободно мест, цена) | Must |
...
| ID | Название | Приоритет |
|----|----------|-----------|
| US-2 | Клиент видит список свободных слотов, чтобы выбрать заезд | High |
...
|----|----------|-----------|
| AC-001 | **Дано** клиент авторизован и открыл таб «Заезды», фильтры в дефолте, **Когда** `listSlots` вернул непустой список, **Тогда** показаны слоты на **ближайшие 7 дней** (дефолт API), каждая карточка содержит дату/время старта, конфигурацию трассы и тип, маршала, цену и «Свободно N из M». | P0 |
...
| ID | Критерий | Приоритет |
|----|----------|-----------|
| AC-E01 | **Дано** в системе нет ни одного предстоящего слота и фильтры в дефолте, **Когда** `listSlots` вернул пустой `items`, **Тогда** показан empty state «Пока нет доступных заездов» с подсказкой «Загляните позже» (микрокопия дизайна RR-D11; без CTA «Изменить фильтры»; обновление — жестом pull-to-refresh). | P1 |
